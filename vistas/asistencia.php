...
<div class="testbox">
  ...
  <form action="../ajax/asistencia.php" method="post">
      <hr>
    <span id="date" name="date"></span>
    <div class="accounttype">
      <input type="radio" value="None" id="radioOne" name="account" checked/>
      <label for="radioOne" class="radio" chec>Entrada</label>
      <input type="radio" value="None" id="radioTwo" name="account" />
      <label for="radioTwo" class="radio">Salida</label>
    </div>
  <hr>
  <label id="icon" for="codigo_emp"><i class="icon-unlock"></i></label>
  <input type="text" name="codigo_emp" id="codigo_emp" placeholder="MT00" onkeyup="nombres()" required/>
...
document.getElementById("hora").value = d + "/" + m + "/" + y+"/"+h;


function nombres() {
    
    var value = document.getElementById('codigo_emp').value;   
   

            if(value='MT02'){
            var name2 = 'ARELY FLORES';
            document.getElementById('name').value=name2;
            }
            if(value='MT04'){
            var name3 = 'DENNIS CORVERA';
